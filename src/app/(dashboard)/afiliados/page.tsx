...
export default async function Page(){
 ...
 const [{data:memberships,error},{data:programs,error:programError}]=await Promise.all([
  ...
  admin?admin.from("affiliate_programs").select("id,mode,products(id,name,producer_id)").eq("active",true).neq("mode","invite").limit(100):Promise.resolve({data:[],error:null}),
 ]);
