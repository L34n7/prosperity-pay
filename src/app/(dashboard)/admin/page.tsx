...
export default async function Page(){const {supabase}=await requireUser();const {data:authorized}=await supabase.rpc("is_finance_admin");if(!authorized)redirect("/dashboard");const admin=createAdminClient();const [identity,pix,withdrawals,payments,webhooks,connections,users,audit]=await Promise.all([
 ...
 ]);const failed=[identity,pix,withdrawals,payments,webhooks,connections,users,audit].find(r=>r.error);if(failed?.error)throw failed.error;return <AdminView identity={identity.data??[]} pix={pix.data??[]} withdrawals={withdrawals.data??[]} payments={payments.data??[]} webhooks={webhooks.data??[]} connections={connections.data??[]} users={users.data??[]} audit={audit.data??[]}/>}
