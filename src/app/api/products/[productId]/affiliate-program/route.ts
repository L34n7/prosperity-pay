...
export async function PUT(request:Request,context:Context){
  try{
    ...
    for(const [offerId,c] of configs){const {error}=await admin.from("offers").update({affiliate_enabled:c.enabled,affiliate_commission_bps:c.commissionBps}).eq("id",offerId).eq("product_id",productId);if(error)throw error}
