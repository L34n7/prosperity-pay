...
export async function PUT(request:Request,context:Context){
  try{
    ...
    const {data:program,error:programError}=await admin.from("affiliate_programs").upsert({product_id:productId,mode:mode as "public"|"approval"|"invite",active,cookie_days:cookieDays,terms,attribution_model:attributionModel,customer_data_access:body.customerDataAccess===true,marketplace_enabled:body.marketplaceEnabled===true,commission_addons:body.commissionAddons===true,commission_prorated_changes:body.commissionProratedChanges===true,support_email:supportEmail,landing_page_url:landingPageUrl,marketplace_description:marketplaceDescription,marketplace_tags:marketplaceTags},{onConflict:"product_id"}).select().single();
