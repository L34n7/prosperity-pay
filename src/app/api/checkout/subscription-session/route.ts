...
import { createSubscriptionSessionCheckout } from "@/lib/checkout/subscription-session-checkout";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createSubscriptionSessionCheckout({
      sessionToken: requiredString(body, "sessionToken", 200),
    ...
    console.error("[subscription-session-checkout] payment attempt failed", error);
