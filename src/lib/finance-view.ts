...
export async function getFinanceView() {
 ...
 const {data:payments,error:paymentsError}=ids.length?await supabase.from("payments").select("id,order_id,status,external_payment_id,external_reference,gross_amount_cents,provider_fee_amount_cents,created_at,paid_at,raw_provider_data,payment_transactions(transaction_type,status,occurred_at,created_at)").in("order_id",ids).order("created_at",{ascending:false}):{data:[],error:null};
 if(paymentsError) throw paymentsError;
 return {orders:(orders??[]).map(order=>({...order,customers:null})),commissions:commissions??[],balance:balance??{pending_cents:0,available_cents:0,withdrawing_cents:0,total_received_cents:0},withdrawals:withdrawals??[],payments:payments??[]};
}
